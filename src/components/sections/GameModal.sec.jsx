...
import { useTimeout, useForm } from 'hooks';
...
const Modal = () => {
    ...
    const [pokeGuess, setGuess, onChangeHandler] = useForm('');
        ...
        switch (type) {
            case 'EMPTY':
                return setResult((prevState) => ({
                    ...prevState,
                    isCorrect: false,
                    error: `Please don't leave an empty blank!`
                }));
            case 'WRONG_INPUT':
                return setResult((prevState) => ({
                    ...prevState,
    ...
    const requester = async (req) => {
        ...
        if (req === 'NEW_POKE') {
            ...
            const pokeNumberArray =
                res.data.pokemon[randomize(res.data.pokemon.length)].pokemon.url.split('/');
    ...
    return createPortal(
        <Dialog initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }}>
            <div className="poke">
                <Image
                    width={200}
                    height={200}
            ...
            <Panel>
                <Typography className="panel_header" as="h2">
                    Guess the pokemon!
                </Typography>
                <Form danger={result.error} success={result.isCorrect} onSubmit={onSubmitHandler}>
                    <div className="input">
                        <TextField
                            ...
                            value={pokeGuess}
                            onChange={(eve) =>
                                onChangeHandler(eve, () => validator('SET_INITIAL'))
                            }>
                            {(result.error || result.isCorrect) && (
                                <Lottie options={animationOptions} height={22} width={22} />
                            )}
                        </TextField>
                        {result.error && (
                            <Typography className="input_error">{result.error}</Typography>
                        )}
                    </div>
                    <Button minWidth="medium" secondary>
                        Submit
                    </Button>
                </Form>
            ...
            <Toast text={toast.toastText} condition={toast.isToast} />
        </Dialog>,
